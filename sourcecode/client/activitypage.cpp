...
*/
    isPersonal = true;
    bigIconButton* personal = new bigIconButton(1, ":/icons/icons/personal-activity.svg", "", "", 0, 0, activityBar);
    personal->setFixedSize(30, 30);
    activityLayout->addWidget(personal);
    bigIconButton* group = new bigIconButton(1, ":/icons/icons/group-activity.svg", "", "", 0, 0, activityBar);
    group->setFixedSize(30, 30);
    group->hide();

    connect(personal, &bigIconButton::clicked, this, [=] {
        activityLayout->removeWidget(personal);
        personal->hide();
        group->show();
        activityLayout->insertWidget(1, group);
        isPersonal = false;
    });

    connect(group, &bigIconButton::clicked, this, [=] {
        activityLayout->removeWidget(group);
        group->hide();
        personal->show();
        activityLayout->insertWidget(1, personal);
        isPersonal = true;
    });
