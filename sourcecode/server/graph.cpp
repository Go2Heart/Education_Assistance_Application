...
void Graph::Init() {
	...
	FILE* in = fopen("/home/ubuntu/Codefield/EduServer/server 4/default_settings/graph.in", "r");
	fscanf(in, "%d %d", &n, &m);
	for(int i = 0; i < n; i++) {
		char name[45];
		fscanf(in, "%s", name);
	...
	in = fopen("/home/ubuntu/Codefield/EduServer/server 4/default_settings/traffic.in", "r");
	int x, y, z;
	fscanf(in, "%d", &x);
	for(int i = 0; i < x; i++)
	{
