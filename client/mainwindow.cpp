...
void MainWindow::mouseMoveEvent(QMouseEvent *event) {
    ...
    if(!mousePressed) {
        mouseState = 0;
        if(!maximized) {
            if(abs(event->pos().x() - ui->mainWidget->pos().x()) < 5)
                mouseState |= AT_LEFT;
            if(abs(event->pos().x() - ui->mainWidget->pos().x() - ui->mainWidget->width()) < 5)
                mouseState |= AT_RIGHT;
            if(abs(event->pos().y() - ui->mainWidget->pos().y()) < 5)
                mouseState |= AT_TOP;
            if(abs(event->pos().y() - ui->mainWidget->pos().y() - ui->mainWidget->height()) < 5)
